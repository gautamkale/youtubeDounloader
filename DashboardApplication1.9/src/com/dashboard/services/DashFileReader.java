		...
		try {
			...
			if (callerName.equals("Incident")) {
				...
				/*	System.out.println("select * from  Ticket_Details2 where (DateDiff('H',#"+ strDate + "#,SLA_Target_Date)<=2) AND (DateDiff('H',#"
							+ strDate + "#,SLA_Target_Date)>=0) and  Business_Service_Name='"+db1.getBusinessServiceName().trim()+"'");*/
					statement.execute("select * from  Ticket_Details2 where (DateDiff('H',#"+ strDate + "#,SLA_Target_Date)<=2) AND (DateDiff('H',#"
							+ strDate + "#,SLA_Target_Date)>=0) and  Business_Service_Name='"+db1.getBusinessServiceName().trim()+"'");
					set = statement.getResultSet();
					while (set.next()) {
	...
	public String updateBs(String bsName,int p1p2, int p3p4, int sr, int fpr)
		...
		try {
			statement = conn.createStatement();
				...
				str="updated";
			//System.out.println("update BS_Details set P1_P2="+p1p2+",P3_P4="+p3p4+",SR="+sr+",FPR="+fpr+" where Business_Service_Name='"+bsName+"' ");
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			str="error";
